...
class BigFile:
    def __init__(self, datadir, bin_file="feature.bin"):
        self.nr_of_images, self.ndim = map(int, open(os.path.join(datadir,'shape.txt')).readline().split())
        ...
        # print ("[%s] %dx%d instances loaded from %s" % (self.__class__.__name__, self.nr_of_images, self.ndim, datadir))
    ...
    def readall(self):
        ...
        return [x[1] for x in index_name_array], [ res[i*self.ndim:(i+1)*self.ndim].tolist() for i in range(nr_of_images) ]
    ...
    def shape(self):
        return [self.nr_of_images, self.ndim]
...
class StreamFile:
    ...
    def next(self):
        if self.current >= self.nr_of_images:
            ...
            res = array.array('f')
            res.fromfile(self.fr, self.ndim)
